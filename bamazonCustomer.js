...
function displayAllProducts() {
     var display =
          'select item_id, product_name, department_name, price, stock_quantity, (price*stock_quantity) AS product_sales from products';
     var query = connection.query(display, function (err, res) {
          if (err) throw err;
     ...
     var query = connection.query(
          ...
          function (err, res) {
               ...
               if (parseFloat(res[0].stock_quantity) >= chosenAmount) {
                    newAmount =
                         parseFloat(res[0].stock_quantity) - chosenAmount;
                    updateAmount();
               } else {
...
function findPrice() {
     var query = connection.query(
          ...
          function (err, res) {
               ...
               console.log(
                    'Product pulled for you! Your total: $' + totalPrice
               );
               customerPrompt();
          }
     );
